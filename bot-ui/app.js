...
function initVizStyleChooser() {
  ...
  const STYLE_SETTINGS_MENU = document.getElementsByClassName(
    "viz-settings-visibility"
  );
...
function initWindmillStyleSettings() {
  ...
  windmill_sliders_values = document.getElementsByClassName(
    "windmill-slider-value"
  );
    ...
    for (let i = 0; i < windmill_sliders.length; i++) {
      if (typeof windmill_sliders[i].noUiSlider != "undefined") {
        if (i === 3 || i === 7 || i === 8 || i === 12 || i === 13) {
          windmill_sliders_values[i].innerHTML = windmill_sliders[
            i
          ].noUiSlider.get();
        } else {
          windmill_sliders_values[i].innerHTML = Math.round(
            windmill_sliders[i].noUiSlider.get()
          );
        }
      }
    }
  ...
  function initPresets() {
    for (let i = 0; i < WINDMILL_PRESETS.length; i++) {
      WINDMILL_PRESETS[i].addEventListener("click", function () {
        ...
        WINDMILL_PRESETS[i].className =
          "windmill-presets generic-menu-item selected";
        let presetSliderSettings;
        switch (i) {
          //	Default
          case 0:
            presetSliderSettings = [
              120,
              120,
              120,
              1,
...
function visualizeWindmill() {
  ...
  function readSliders() {
    ...
    if (windmill_checkboxes[0].checked === false) {
      document.getElementById("hidden-windmill-sliders").style =
        "display: block;";
    ...
    for (let i = 1; i < 50; i += 1) {
      ...
      for (let j = 1; j < 7; j++) {
        ...
        canvasCtx.arc(
          ...
          HEIGHT / 2,
          radius + j * bladeWidth,
          arcStart + ((Math.PI * 2) / 4) * 2,
          arcEnd + ((Math.PI * 2) / 4) * 2
        );
        canvasCtx.stroke();
        ...
        canvasCtx.arc(
          ...
          HEIGHT / 2,
          radius + j * bladeWidth,
          arcStart + ((Math.PI * 2) / 4) * 3,
          arcEnd + ((Math.PI * 2) / 4) * 3
        );
        canvasCtx.stroke();
      }
    }
  ...
  function draw() {
    ...
    canvasCtx.canvas.width = WIDTH = "2000";
    canvasCtx.canvas.height = HEIGHT = "2000";
...
function initFreqBarsStyleSettings() {
  ...
  freq_bars_checkboxes = document.getElementsByClassName(
    "freq-bars-responsive"
  );
    ...
    for (let i = 0; i < freq_bars_sliders.length; i++) {
      if (typeof freq_bars_sliders[i].noUiSlider != "undefined") {
        if (i === 3 || i === 7 || i === 11 || i === 13) {
          ...
          freq_bars_values[i].innerHTML =
            Math.pow(2, Math.round(freq_bars_sliders[i].noUiSlider.get())) / 2;
        } else {
          freq_bars_values[i].innerHTML = Math.round(
            freq_bars_sliders[i].noUiSlider.get()
          );
        }
      }
    }
...
function visualizeFrequencyBars() {
  ...
  function draw() {
    ...
    for (let i = 0; i < bufferLength; i++) {
      ...
      barHeight =
        Math.round(barHeight_setting * Math.max(dataArray[i] * 4 - 250, 0)) +
        10;
      canvasCtx.fillStyle =
        "rgba(" +
        (dataArray[i] * responsiveBarColor_r + barColor_r) +
        "," +
        (dataArray[i] * responsiveBarColor_g + barColor_g) +
        ", " +
        (dataArray[i] * responsiveBarColor_b + barColor_b) +
        ", " +
        barColor_a +
        ")";
      canvasCtx.fillRect(
        x,
        HEIGHT - barHeight / 2 - HEIGHT / 2,
        barWidth,
        barHeight
      );
      x += barWidth + 1;
